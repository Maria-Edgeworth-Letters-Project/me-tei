...
```  
{{if(isBlank(cells['ID'].value), '', '<place xml:id="' + cells['ID'].value + '">' + '<placeName>' + cells['Place Name'].value + '</placeName>' + if(isBlank(cells['coordinates'].value), '', '<location><geo>' + cells['coordinates'].value + '</geo></location>')+ if(isBlank(cells['Geonames URI'].value), '', '<idno type="URI">' + cells['Geonames URI'].value + '</idno>') + '</place>')}}
